...
void PhyphoxBLE::configReceived(const GattWriteCallbackParams *params)
{
    ...
    if (params->handle == PhyphoxBLE::OsziCharacteristic.getValueHandle() ) {
        if(OsziHandler != nullptr){
...
void PhyphoxBLE::readOszi(uint8_t *arrayPointer, unsigned int arraySize)
{
	uint16_t myArraySize = arraySize;
	ble.gattServer().read( OsziCharacteristic.getValueHandle(), arrayPointer, &myArraySize);
}
...
void PhyphoxBLE::writeOld(float& f1, float& f2, float& f3, float& f4)
{
	...
	float array[4] = {f1, f2, f3, f4};
	data = reinterpret_cast<uint8_t*>(array);
	ble.gattServer().write(OsziCharacteristic.getValueHandle(), data, 16);
}
